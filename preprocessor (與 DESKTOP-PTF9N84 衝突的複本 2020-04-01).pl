...
for (0..$#temp){$temp[$_] -= 1;}
chomp( $system_para->{typeID} = [@temp]);# type IDs in compositional units 
...
chomp( $system_para->{types} = [@temp]);# total number of compositional units
...
my $aveEnePerUnit = $eneSum/($system_para->{dataNo}*$system_para->{compNo}); # 5 times for all upper bounds
...
for (my $str=0;$str<$system_para->{compNo};$str++){
	for (0  $system_para->{typeID}my $type=0;$type<$system_para->{compNo};$str++){
...
#my $compNo = $system_para->{compNo}; # scalar reference
#
#for (my $str=0;$str<$system_para->{compNo};$str++){
#	for (my $type=0;$type<=$#{$system_para->{typeID}};$type++){
#	
#			chomp ( $refstr->[$dataID]->[$str] =  $input_array[4+$dataID*($system_para->{compNo}+1)+($str+1)] -1 );#$str starts from 0 (-1 is to make type from 0)
#	}
#}
