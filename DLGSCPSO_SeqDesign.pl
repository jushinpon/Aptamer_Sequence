...
for (my $i=0; $i<$Number_of_particles; $i++){ 
    ...
    for (my $j=0; $j < $dimension; $j++){
        my $pbval = ($pBest[$i][$j] - $x[$i][$j]);
        my $lbval = ($plBest[$i][$j] - $x[$i][$j]);
        my $gbval = ($gBest[$j] - $x[$i][$j]);

        my @changeType;
        if($pbval != 0){
          $changeType[0] = $pBest[$i][$j];  
        }else{
          $changeType[0] = $x[$i][$j];
        }

        if($lbval != 0){
          $changeType[1] = $plBest[$i][$j];
          $changeType[2] = $changeType[1]; 
        }else{
          $changeType[1] = $x[$i][$j];
          $changeType[2] = $changeType[1];
        }
        ...
        if($gbval != 0){
          $changeType[3] = $gBest[$j];  
        }else{
          $changeType[3] = $x[$i][$j];
        }
		
        $changeType[4] = int(rand(4));
        if($changeType[4] > 3){die "Base type:$changeType[3], wrong (larger than 3)!";}
        my $tempid = int(rand(5));
        $x[$i][$j] =  $changeType[$tempid];
