...
require './PSO_fitness_SeqDesign.pl'; # get the fitness for each iteration
require './preprocessor_SeqDesign.pl';#reference data reading and PSO setting
require './keepBetter_SeqDesign.pl';#reference data reading and PSO setting

##### remove old files first
my @oldfiles = <00BestSeq_I*_P*.dat>;
for (@oldfiles){
	unlink $_;
	print "remove $_\n"; 	
}
...
my %para_set;# all parameters (keys) and their energy values (values)
#system parameters
my %system_para;
...
&preprocessor_SeqDesign(\%system_para,\%para_set);
...
my $Number_of_particles = 20;##@ p.132 c = 0.5 + ln2
...
my $dimension =  $system_para{compNo};### parameter number to be fitted
#print "dimension: $dimension\n";
open my $summary, ">SeqDesign_summary.dat";
...
## setting initial values for all dimensions	
    for (my $j=0; $j < $dimension; $j++){  	
      $x[$i][$j]=int(rand(4)); ###initial values for parameters 
...
### get the fitness here
     ...
     $fitness[$i] = &PSO_fitness_SeqDesign(\@{$x[$i]},\@fittedData,\%para_set);
...
if ($fitness[$lowestfitID] < $gfBest){
	...
	&keepBetter_SeqDesign(\%system_para,$gfBest,$lowestfitID,$iteration,\@gBest,$summary); #passing ram address
} 
...
for (my $i=0; $i<$Number_of_particles; $i++){ 
    ...
    for (my $j=0; $j < $dimension; $j++){
        my @basearray;
            $basearray[0] =  $pBest[$i][$j];
            $basearray[1] =  $plBest[$i][$j];
            $basearray[2] = $basearray[1];
            $basearray[3] = $gBest[$j];
            $basearray[4] = $basearray[3];
            $basearray[5] = $basearray[3];
            $basearray[6] = int (rand(4));
            $x[$i][$j] = $basearray[int(rand(7))];
